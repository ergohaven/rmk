...
pub const BATTERY_PERCENT_UNKNOWN: u8 = 0xff;

static LOCAL_BATTERY_PERCENT: AtomicU8 = AtomicU8::new(BATTERY_PERCENT_UNKNOWN);
static REMOTE_BATTERY_PERCENT: AtomicU8 = AtomicU8::new(BATTERY_PERCENT_UNKNOWN);

pub fn set_local_battery_percent(level: u8) {
    LOCAL_BATTERY_PERCENT.store(level.min(100), Ordering::Release);
}

pub fn set_remote_battery_percent(level: u8) {
...
pub fn battery_halves_percent() -> (u8, u8) {
    (
        ...
        REMOTE_BATTERY_PERCENT.load(Ordering::Acquire),
    )
}
    ...
    async fn process(&mut self, event: Event) -> ProcessResult {
        match event {
            Event::Battery(val) => {
                ...
                {
                    #[cfg(feature = "controller")]
                    send_controller_event(&mut self.controller_pub, ControllerEvent::BatteryAdc(val));
                    ...
                    if matches!(self.battery_state, BatteryState::Normal(_) | BatteryState::NotAvailable) {
                        ...
                        set_local_battery_percent(battery_percent);
